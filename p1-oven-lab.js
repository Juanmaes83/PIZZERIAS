(() => {
  ...
  async function initPremiumOven(){
    const uri=window.PremiumOvenMasterDataUri;
    const validSource=Boolean(uri&&uri.startsWith('data:image'));
    document.body.classList.remove('has-premium-oven','oven-load-failed');
    $('#asset-label').textContent='OVEN LOADING';
    $('#telemetry-oven').textContent='loading';

    if(!validSource){
      document.body.classList.add('oven-load-failed');
      $('#asset-label').textContent='OVEN FALLBACK';
      $('#telemetry-oven').textContent='missing-source';
      return false;
    }

    try{
      premiumOvenImage.src=uri;
      if(typeof premiumOvenImage.decode==='function'){
        await premiumOvenImage.decode();
      }else if(!premiumOvenImage.complete||premiumOvenImage.naturalWidth===0){
        await new Promise((resolve,reject)=>{
          premiumOvenImage.addEventListener('load',resolve,{once:true});
          premiumOvenImage.addEventListener('error',reject,{once:true});
        });
      }
      if(!premiumOvenImage.complete||premiumOvenImage.naturalWidth<1||premiumOvenImage.naturalHeight<1){
        throw new Error('Premium oven decoded with invalid dimensions');
      }
      ...
      console.info('[P1.3] Premium Oven Master ready',premiumOvenImage.naturalWidth,premiumOvenImage.naturalHeight);
      return true;
    }catch(error){
      ...
      console.error('[P1.3] Premium Oven Master failed to render',error);
      return false;
    }
  }
  ...
  function play(){
    ...
    schedule(()=>{setState('APPROACH');peel.style.opacity='1';pose('translate(28%,3%) rotate(-1deg) scale(.88)','.38s')},220);
    schedule(()=>{setState('PEEL_ENTER');pose('translate(2%,-14%) rotate(-.4deg) scale(.74)','.48s','cubic-bezier(.32,.04,.28,1)')},600);
    schedule(()=>{setState('CONTACT');stage.classList.add('is-contacting','is-occluding');peel.style.filter='brightness(.64) saturate(.82)';pose('translate(-12%,-20%) rotate(0deg) scale(.68)','.22s','cubic-bezier(.2,.72,.3,1)')},1080);
    schedule(()=>{setState('MICRO_ADJUST');pose('translate(-14%,-19%) rotate(.7deg) scale(.68)','.16s','cubic-bezier(.3,.1,.5,1)')},1300);
    schedule(()=>{setState('LOAD');peelPizza.src=src;peelPizza.alt=p.name;peelPizza.style.transition='opacity .14s ease,transform .24s cubic-bezier(.18,.78,.24,1)';peelPizza.style.opacity='1';peelPizza.style.transform='scale(.98)'},1450);
    schedule(()=>{stage.classList.remove('is-occluding');peel.style.filter='brightness(.82)';setState('EXTRACT');pose('translate(-5%,-15%) rotate(.15deg) scale(.72)','.28s','cubic-bezier(.12,.7,.22,1)')},1630);
    schedule(()=>{stage.classList.remove('is-contacting');peel.style.filter='none';pose('translate(17%,-2%) rotate(-.3deg) scale(.84)','.42s','cubic-bezier(.16,.72,.2,1)')},1910);
    schedule(()=>{pose('translate(36%,11%) rotate(-.8deg) scale(1.0)','.42s','cubic-bezier(.18,.72,.2,1)');peelPizza.style.transform='scale(1.03)'},2330);
    schedule(()=>{setState('PRESENT');stage.classList.add('is-delivering');pose('translate(42%,17%) rotate(-1deg) scale(1.04)','.18s','cubic-bezier(.2,.72,.28,1)')},2750);
    schedule(()=>{setState('FOREGROUND_REVEAL');revealProduct()},3100);
    schedule(()=>{peel.style.transition='transform .46s cubic-bezier(.25,.05,.5,1),opacity .38s ease';peel.style.transform='translate(42%,34%) rotate(-1.4deg) scale(1.02)';peel.style.opacity='0'},3260);
    schedule(()=>{setState('PRODUCT_READY');stage.classList.remove('is-preparing','is-delivering');stage.classList.add('is-product-ready');running=false},3720);
  }
